...
;the testing file for Richard C. Waters' XP pretty printer.
...
;tests of formats for various special forms.
 ...
 X)")
  ...
  ((ftest 100 0 '(let ((a (foo 3)) (b (foo 4)) (c 1)) (tuz a b)))
   "(LET ((A (FOO 3)) (B (FOO 4)) (C 1)) (TUZ A B))")
  ((ftest 41 0 '(let ((a (foo 3)) (b (foo 4)) (c 1)) (tuz a b)))
   "(LET ((A (FOO 3)) (B (FOO 4)) (C 1))
  (TUZ A B))")
  ((ftest 35 0 '(let ((a (foo 3)) (b (foo 4)) (c 1)) (tuz a b)))
   "(LET ((A (FOO 3))
      (B (FOO 4))
      (C 1))
  (TUZ A B))")
  ((ftest 44 50 '(let ((a (foo 3)) (b (foo 4)) (c 1)) (tuz a b)))
   "(LET
 ((A (FOO 3)) (B (FOO 4)) (C 1))
 (TUZ A B))")
  ((ftest 21 0 '(let (bar baz def gack gortch) (tuz a b)))
   "(LET (BAR BAZ DEF
      GACK GORTCH)
  (TUZ A B))")
  ((ftest 31 0 '(let ((bar baz def gack . gortch))))
   "(LET ((BAR BAZ DEF GACK
       . GORTCH)))")
  ((ftest 100 0 '(let (bar baz def gack . gortch)))
   "(LET (BAR BAZ DEF GACK . GORTCH))")
  ((ftest 40 0 '(let foo))
   "(LET FOO)")
  ((ftest 40 0 '(let ()))
   "(LET ())")
...
"(COND)")
  ...
  ((ftest 30 0 '(defconstant foo 2 (defconstant)) (*print-level* 1))
   "(DEFCONSTANT FOO 2 #)")
  ((ftest 40 0 '(define-setf-expander ldb (a b) (make-right body a b)))
   "(DEFINE-SETF-EXPANDER LDB (A B)
  (MAKE-RIGHT BODY A B))")
 ...
 (LIST B C))")
  ...
  (CAR X)
  (MAKE-BODY A B))")
  ((ftest 40 0 '(define-setf-expander bar (a b) (car x) (make-body a b)))
   "(DEFINE-SETF-EXPANDER BAR (A B)
  (CAR X)
  (MAKE-BODY A B))")
 ...
 (PRINT A))")
  ...
  ((ftest 20 0 '(etypecase type (:foo (print 3))))
   "(ETYPECASE TYPE
  (:FOO (PRINT 3)))")
  ((ftest 50 0 '(eval-when (:compile-toplevel :load-toplevel) (defun foo () (car x))))
